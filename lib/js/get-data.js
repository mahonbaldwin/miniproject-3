...
//function to display list of medications
    ...
    function timeOfDay(di){
        if(di.timing.repeat.timeOfDay) {
            return "at " + di.timing.repeat.timeOfDay
        } else {
            return "";
        }
    }

    function doseFrequency(di){
        if(di.timing.repeat.frequency) {
            return di.timing.repeat.frequency + " time(s) every ";
        } else {
            return "";
        }
    }
    ...
    function dosageInfo(di) {
        ...
        dosage += doseFrequency(di) +
            (di.timing.repeat.period || di.timing.repeat.dayOfWeek || "") + " " + dosagePeriods(di.timing.repeat.periodUnit) +
            timeOfDay(di)
